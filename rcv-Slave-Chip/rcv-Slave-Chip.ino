...
const int TrimingPin = 4;   // TrimingPin D3
...
// Function to split a message into parts using a specified delimiter
...
void handleSerial() {
  if (Serial.available()) {
    ...
    String parts[4];  // Gear, speed, Stearing angle, Trim angle
    ...
    if (n > 2) {
      ...
      // Set the position of the servo motors based on received angles
      Stear.write(parts[2].toInt());
      Trim.write(parts[3].toInt());
    }
  }
  // Stop the motor after a delay if it's in the neutral state
  if (millis() - RNTime > 3000 && running == "N") {
    digitalWrite(MoD1, LOW);
    digitalWrite(MoD2, LOW);
  }
}
// Function to calibrate the GY271 compass
void calibrateGY271() {
...
// Function to handle the GY271 compass data
void handleGY271() {
  int x, y, z, a;
  String cardinal;
  qmc.read(&x, &y, &z);
  ...
  // Adjust the heading to be within the range [0, 360)
  if (headingDegrees < 0) {
    headingDegrees += 360;
  }
  // Determine cardinal direction based on the heading
  ...
  // Print the heading and cardinal direction
...
// Function to handle MPU6050 data
void handleMPU() {
  ...
  // Get calibrated values for accelerometer
...
// Function to initialize pins and set initial motor and servo positions
void initPins() {
  pinMode(MoD1, OUTPUT);
  digitalWrite(MoD1, LOW);
  pinMode(MoD2, OUTPUT);
  ...
  Trim.attach(TrimingPin, 500, 2500);
  Stear.write(90);
  Trim.write(90);
...
// Function for a smart delay that allows serial data to be processed during the delay
